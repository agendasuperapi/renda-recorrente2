-- Primeiro, vamos criar uma nova função que pode ser chamada tanto em INSERT quanto em UPDATE
...
BEGIN
  -- Para UPDATE: só processa se o sync_status mudou para 'pending'
  IF TG_OP = 'UPDATE' THEN
    IF NOT (OLD.sync_status IS DISTINCT FROM NEW.sync_status AND NEW.sync_status = 'pending') THEN
      RETURN NEW;
    END IF;
  END IF;

  -- Buscar informações do usuário (incluindo avatar_url)
  SELECT 
    p.id,
    p.name,
    p.email,
  ...
  -- Inserir ou atualizar unified_users (incluindo avatar_url)
  ...
  DO UPDATE SET
    ...
    avatar_url = COALESCE(EXCLUDED.avatar_url, unified_users.avatar_url),
...
DROP TRIGGER IF EXISTS sync_local_payment_to_unified_trigger ON public.payments;

-- Criar novo trigger que executa em INSERT e UPDATE
CREATE TRIGGER sync_local_payment_to_unified_trigger
  AFTER INSERT OR UPDATE ON public.payments
  FOR EACH ROW
  EXECUTE FUNCTION sync_local_payment_to_unified();