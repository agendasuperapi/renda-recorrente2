  ...
  const ContentComponent = () => (
    <>
      {loadingProfile ? (
        ...
        <ScrollArea className={isMobile ? "h-[calc(95vh-140px)]" : "h-[calc(90vh-120px)] px-6 lg:px-0"}>
          <div className={`space-y-6 pb-6 ${isMobile ? "px-4" : ""}`}>
            ...
            <Tabs defaultValue="personal" className="w-full">
              <TabsList className="w-full grid grid-cols-4">
                ...
                <TabsTrigger value="commissions" className="flex items-center gap-2 text-xs">
                ...
                <Card>
                  ...
                  <CardContent>
                    {subscription ? (
                      ...
                      <p className="text-muted-foreground text-center py-4">Nenhum plano ativo</p>
                    )}
                  </CardContent>
                </Card>
              </TabsContent>
                            ...
                            <CardContent className="p-4">
                              <div className="flex justify-between items-start gap-4">
                                <div className="flex-1">
                                  <p className="font-medium">{(payment.plans as any)?.name || "Plano"}</p>
                                  <p className="text-sm text-muted-foreground">
                                    {payment.payment_date ? format(new Date(payment.payment_date), "dd/MM/yyyy HH:mm", { locale: ptBR }) : "-"}
                                  </p>
                                </div>
                        ...
                        <div className="mb-4">
                          <p className="text-sm text-muted-foreground">
                            Total: <span className="font-bold text-foreground">
                              {commissions.reduce((acc, c) => acc + Number(c.amount || 0), 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                            </span>
                          </p>
                        </div>
                        <div className="space-y-3">
                                ...
                                <div className="flex justify-between items-start gap-4">
                                  <div className="flex-1">
                                    <p className="font-medium text-sm">
                                      {commission.commission_type === "subscription" ? "Assinatura" : "Comissão"}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {commission.created_at ? format(new Date(commission.created_at), "dd/MM/yyyy HH:mm", { locale: ptBR }) : "-"}
                                    </p>
                                    <Badge variant="outline" className="mt-2 text-xs">
                                      {commission.status === "available" ? "Disponível" : 
                                       commission.status === "withdrawn" ? "Sacado" : 
                                       commission.status === "pending" ? "Pendente" : commission.status}
                                    </Badge>
                                  </div>
                                  <div className="text-right">
                                    <p className="font-bold">
                                      {Number(commission.amount || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
