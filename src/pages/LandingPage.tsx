  ...
  const checkAdminRole = async (userId: string) => {
    try {
      const {
        data,
        error
      } = await supabase.from('user_roles').select('role').eq('user_id', userId).eq('role', 'super_admin').single();
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      localStorage.setItem('lastUsedCoupon', JSON.stringify({
        ...
        // Código exatamente como foi digitado (custom_code se for de afiliado)
        custom_code: couponResult.custom_code || null,
        // Guardar custom_code para referência
        data: couponData
      }));
      toast({
        title: "Cupom válido!",
  ...
  return <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto flex h-16 items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {showHomeButton && <Button onClick={() => scrollToSection("inicio")} variant={activeSection === "inicio" ? "secondary" : "ghost"} size="sm" className="transition-all duration-300">
                  Início
                </Button>}
              <Button onClick={() => scrollToSection("como-funciona")} variant={activeSection === "como-funciona" ? "secondary" : "ghost"} size="sm" className="transition-all duration-300">
                Como funciona
              </Button>
              <Button onClick={() => scrollToSection("vantagens")} variant={activeSection === "vantagens" ? "secondary" : "ghost"} size="sm" className="transition-all duration-300">
                Vantagens
              </Button>
              {products.length > 0 && <Button onClick={() => scrollToSection("produtos")} variant={activeSection === "produtos" ? "secondary" : "ghost"} size="sm" className="transition-all duration-300">
                  Produtos
                </Button>}
              <Button onClick={() => scrollToSection("planos")} variant={activeSection === "planos" ? "secondary" : "default"} size="sm" className="transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105">
                Quero contratar
              </Button>
            </nav>
      ...
      backgroundColor: announcementBanner.background_color
          ...
          <div className="container mx-auto flex flex-col md:flex-row items-center justify-center gap-3 md:gap-4">
            ...
            __html: announcementBanner.text
          }} style={{
            color: announcementBanner.text_color
          }} />}
              {announcementBanner.subtitle && <div className="text-xs md:text-sm mt-1 rich-text-banner" dangerouslySetInnerHTML={{
            __html: announcementBanner.subtitle
          }} style={{
            color: announcementBanner.text_color
          }} />}
            </div>
            {announcementBanner.button_text && announcementBanner.button_url && <Button size="sm" onClick={() => {
        ...
        <div className="container mx-auto max-w-6xl px-0">
          <div className="grid md:grid-cols-2 gap-6 md:gap-12 items-center">
            <div className={`w-full px-3 md:px-0 text-center md:text-left transition-all duration-700 ${visibleSections.has('hero') ? 'animate-fade-in' : 'opacity-0 translate-y-10'}`}>
              ...
              <p className="text-base sm:text-lg md:text-xl mb-6 md:mb-8" style={{
              color: getTextColor('hero')
            }}>
                Temos vários produtos para você indicar e criar uma renda recorrente.
              </p>
              <Button size="lg" onClick={() => scrollToSection("planos")} className="w-full md:w-auto text-base md:text-lg px-6 md:px-8 mb-6 md:mb-8 relative overflow-hidden group hover:shadow-2xl transition-all duration-300 before:absolute before:inset-0 before:bg-primary/20 before:blur-xl before:animate-pulse">
                Começar Agora
                <Target className="ml-2 w-4 h-4 md:w-5 md:h-5 group-hover:rotate-12 transition-transform duration-300" />
              </Button>
              <div className="flex justify-center md:justify-start">
                {getHeroImage('Trust Badges') && <img src={getHeroImage('Trust Badges')} alt={heroImages.find(img => img.name === 'Trust Badges')?.alt_text || 'Selos de segurança'} className="h-10 sm:h-12" loading="eager" />}
              </div>
            </div>
            <div className="flex justify-center animate-fade-in" style={{
            animationDelay: '200ms'
          }}>
              {getHeroImage('Hero Person') && <img src={getHeroImage('Hero Person')} alt={heroImages.find(img => img.name === 'Hero Person')?.alt_text || 'Afiliado'} className="w-full max-w-xs sm:max-w-sm md:max-w-md animate-fade-in" style={{
      ...
      <section id="comissao-recorrente" className="py-12 md:py-16 lg:py-20 px-3 md:px-6 relative" style={getGradientStyle('comissao-recorrente')}>
        ...
        <div className={`container mx-auto max-w-4xl px-0 text-center transition-all duration-700 ${visibleSections.has('comissao-recorrente') ? 'animate-fade-in' : 'opacity-0 translate-y-10'}`}>
          ...
          <p className="text-base sm:text-lg md:text-xl mb-6 md:mb-8 px-3 md:px-0" style={{
          color: getTextColor('comissao-recorrente')
        }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 px-3 md:px-0">
            <Card className={`transition-all duration-700 ${visibleSections.has('como-funciona') ? 'animate-fade-in opacity-100' : 'opacity-0 translate-y-10'}`}>
              ...
              <CardContent>
                ...
                color: getTextColor('como-funciona')
              }}>
      ...
      <section id="painel-afiliado" className="py-12 md:py-16 lg:py-20 px-3 md:px-6 relative" style={getGradientStyle('painel-afiliado')}>
        ...
        <div className={`container mx-auto max-w-6xl px-0 transition-all duration-700 ${visibleSections.has('painel-afiliado') ? 'animate-fade-in' : 'opacity-0 translate-y-10'}`}>
          ...
          <p className="text-base sm:text-lg md:text-xl text-center text-muted-foreground mb-8 md:mb-12 px-3 md:px-0" style={{
          color: getTextColor('painel-afiliado')
        }}>
        ...
        <div className={`container mx-auto max-w-6xl px-0 transition-all duration-700 ${visibleSections.has('funcionalidades') ? 'animate-fade-in' : 'opacity-0 translate-y-10'}`}>
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center px-3 md:px-0">
            ...
            <div className="order-2 md:order-1">
              {getHeroImage('Funcionalidades') && <img src={getHeroImage('Funcionalidades')} alt="Dashboard do Sistema" className="w-full h-auto" />}
            </div>

            {/* Lista de Funcionalidades */}
            <div className="order-1 md:order-2 space-y-6 md:space-y-8">
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-6 md:mb-8" style={{
              color: getHeadingColor('funcionalidades')
            }}>
                Funcionalidades
              </h2>
              ...
              <div className="space-y-3 md:space-y-4">
                ...
                return <div key={feature.id} className={`flex items-center gap-4 transition-all duration-700 delay-${index * 100}`} style={{
                  transitionDelay: `${index * 100}ms`
                    ...
                    color: getTextColor('funcionalidades')
                  }}>{feature.name}</p>
                    </div>;
              })}
              </div>

              <p className="text-base text-muted-foreground mt-8" style={{
              color: getTextColor('funcionalidades')
            }}>
      ...
      <section id="layout-responsivo" className="py-12 md:py-16 lg:py-20 px-3 md:px-6 relative" style={getGradientStyle('layout-responsivo')}>
        ...
        <div className={`container mx-auto max-w-6xl px-0 text-center transition-all duration-700 ${visibleSections.has('layout-responsivo') ? 'animate-fade-in' : 'opacity-0 translate-y-10'}`}>
          ...
          <p className="text-base sm:text-lg md:text-xl mb-8 md:mb-12 px-3 md:px-0" style={{
          color: getTextColor('layout-responsivo')
        }}>
          ...
          <div className="flex justify-center px-3 md:px-0">
            {getHeroImage('Responsivo') && <img src={getHeroImage('Responsivo')} alt={heroImages.find(img => img.name === 'Responsivo')?.alt_text || 'APP em diversos dispositivos'} className="w-full max-w-full md:max-w-3xl lg:max-w-4xl object-contain animate-fade-in" style={{
            animationDelay: '200ms'
          }} />}
          </div>
        </div>
      ...
      {products.length > 0 && <section id="produtos" className="py-12 md:py-16 lg:py-20 px-3 md:px-6 relative" style={getGradientStyle('produtos')}>
          ...
          <div className="container mx-auto max-w-6xl px-0">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-8 md:mb-12 px-3 md:px-0" style={{
          color: getHeadingColor('produtos')
        }}>
              Produtos Disponíveis para Afiliação
            </h2>
            <p className="text-base sm:text-lg md:text-xl text-center mb-8 md:mb-12 px-3 md:px-0" style={{
          color: getTextColor('produtos')
        }}>
        ...
        <div className={`container mx-auto max-w-6xl px-0 transition-all duration-700 ${visibleSections.has('planos') ? 'animate-fade-in' : 'opacity-0 translate-y-10'}`}>
                ...
                {validatedCoupon && <div className="border rounded-lg p-4 bg-muted/50 animate-fade-in">
                    ...
                    <div className="flex flex-col md:hidden items-center gap-4">
                      ...
                      <div className="flex flex-col items-center gap-3 text-center w-full">
                        ...
                        {validatedCoupon.description && <p className="text-lg text-muted-foreground">
                    ...
                    {validatedCoupon.affiliate && <div className="hidden md:flex items-start gap-4">
                        <Avatar>
                          <AvatarImage src={validatedCoupon.affiliate.avatar_url} />
                          <AvatarFallback>
                            {validatedCoupon.affiliate.name?.charAt(0) || "A"}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 px-3 md:px-0 max-w-7xl mx-auto">
            ...
            return <div key={plan.id} className={`relative bg-[#2a2a2a] rounded-2xl p-6 md:p-8 shadow-xl transition-all duration-700 hover:scale-105 flex flex-col ${index === 0 ? '' : 'delay-100'} ${visibleSections.has('planos') ? 'animate-fade-in opacity-100' : 'opacity-0 translate-y-10'}`}>
                  ...
                  const isIncluded = hasPlanFeatures ? plan.plan_features!.some(pf => pf.feature_id === feature.id) : false;
                  return <li key={feature.id} className="flex items-start gap-3 transition-all duration-300 hover:translate-x-2 hover:scale-105 cursor-default">
                          ...
                          <span className={`text-sm md:text-base transition-colors duration-300 ${isIncluded ? "text-white" : "text-red-500 line-through"}`}>
                            {feature.name}
                          </span>
                        </li>;
                })}
                  ...
                  <div className="mt-auto">
                    {/* Preço */}
                    <div className="mb-6">
                      {isPro && plan.original_price && plan.original_price > plan.price ? <div className="text-center mb-2">
                        ...
                        </div> : <div className="h-6 mb-2"></div>}
                      <div className="text-center">
                        {plan.is_free ? <span className="text-[#22c55e] text-5xl md:text-6xl font-bold">
                            FREE
                          </span> : <>
                      ...
                      {isPro && plan.original_price && plan.original_price > plan.price ? <p className="text-white text-center text-sm mt-2">
                          {(plan.original_price - plan.price).toFixed(2)} de desconto
                        </p> : <div className="h-5 mt-2"></div>}
                    </div>

                    {/* Botão */}
                    <Button className="w-full bg-[#86efac] hover:bg-[#4ade80] text-gray-900 font-semibold py-6 rounded-lg transition-all duration-300" onClick={() => {
                  ...
                  const trialDays = plan.features && Array.isArray(plan.features) ? plan.features.find((f: string) => {
                  ...
                  const hasCouponWithFreeDays = validatedCoupon && (validatedCoupon.type === 'days' || validatedCoupon.type === 'free_trial');
                  return days > 0 && !hasCouponWithFreeDays ? <p className="text-[#22c55e] text-center text-sm font-medium mt-4">
                          Teste Grátis por {days} {days === 1 ? 'dia' : 'dias'}
                        </p> : <div className="h-5 mt-4"></div>;
                })()}
                  </div>
                </div>;
          })}
      ...
      <footer className="border-t border-border py-8 md:py-12 px-3 md:px-6">
        <div className="container mx-auto max-w-6xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mb-6 md:mb-8">
            <div>
              {getHeroImage('Logo Alternativo') && <img src={getHeroImage('Logo Alternativo')} alt={heroImages.find(img => img.name === 'Logo Alternativo')?.alt_text || 'Logo'} className="h-6 md:h-8 mb-3 md:mb-4" />}
      ...
      {/* Gradient Editor */}
      {isAdmin && editingBlock && <GradientEditor blockName={editingBlock} config={gradientConfigs[editingBlock] || {
      ...
      setGradientConfigs(prev => ({
        ...prev,
        [config.block_name]: config
      }));
