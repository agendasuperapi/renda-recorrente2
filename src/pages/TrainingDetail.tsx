...
const TrainingDetail = () => {
  ...
  useEffect(() => {
    const channel = supabase.channel('training-detail-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'trainings'
    }, () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("training_lessons").select("*").eq("training_id", trainingId).eq("is_active", true).order("order_position");
      if (error) throw error;
      return data;
    },
    enabled: !!trainingId
  ...
  const {
    ...
    queryFn: async () => {
      const {
        count,
        error
      } = await supabase.from("trainings").select("id", {
        count: "exact",
        head: true
      }).eq("category_id", training?.category_id).eq("is_published", true).eq("is_active", true);
      if (error) throw error;
      return count || 0;
    },
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("training_progress").select("lesson_id, is_completed").eq("user_id", userId).eq("training_id", trainingId);
      if (error) throw error;
      return data;
    },
  ...
  return <div className="space-y-6 -mt-6 md:-mt-8 -mx-4 sm:-mx-6 lg:-mx-8">
      ...
      <div className="relative">
        ...
        backgroundImage: bannerImage ? `url(${bannerImage})` : 'linear-gradient(135deg, hsl(var(--primary)) 0%, hsl(var(--primary)/0.8) 100%)'
      }}>
          ...
          {training.banner_text_config && <>
            ...
            opacity: ((training.banner_text_config as any)?.overlayOpacity ?? 40) / 100
              ...
              {((training.banner_text_config as any)?.title || (training.banner_text_config as any)?.subtitle) && <div className={`absolute inset-0 flex flex-col justify-center px-6 sm:px-10 lg:px-16 ${(training.banner_text_config as any)?.textAlign === "left" ? "items-start text-left" : (training.banner_text_config as any)?.textAlign === "right" ? "items-end text-right" : "items-center text-center"}`}>
                  {(training.banner_text_config as any)?.title && <h4 className="text-lg sm:text-xl md:text-2xl font-bold leading-tight" style={{
              color: (training.banner_text_config as any)?.textColor || "#ffffff"
            }}>
                      {(training.banner_text_config as any).title}
                    </h4>}
                  {(training.banner_text_config as any)?.subtitle && <p className="mt-1 text-sm sm:text-base opacity-90 line-clamp-2" style={{
              color: (training.banner_text_config as any)?.textColor || "#ffffff"
            }}>
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
                    ...
                    {training.estimated_duration_minutes > 0 && <span className="flex items-center gap-1">
                        <Clock className="h-4 w-4" />
                        {training.estimated_duration_minutes} min
                      </span>}
                    {Number(avgRating) > 0 && <span className="flex items-center gap-1">
                  ...
                  <div className="max-w-md">
                    ...
                    {progress.percentage === 100 && <div className="flex items-center gap-2 mt-2 text-green-500">
                ...
                {/* Quick Start Button */}
                {progress.percentage < 100 && <Button asChild size="lg" className="md:self-start">
      ...
      <div className="px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        {/* Lessons List */}
        {lessonsLoading ? <div className="space-y-4">
            {[1, 2, 3, 4].map(i => <Skeleton key={i} className="h-24 w-full" />)}
          </div> : lessons?.length === 0 ? <div className="text-center py-12 border rounded-lg">
          ...
          return <Link key={lesson.id} to={locked ? '#' : `/user/training/lesson/${lesson.id}`} className={`block group ${locked ? 'cursor-not-allowed' : ''}`} onClick={e => locked && e.preventDefault()}>
                  <Card className={`overflow-hidden transition-all ${locked ? 'opacity-60' : 'hover:shadow-xl group-hover:scale-[1.02]'} ${completed ? 'ring-2 ring-green-500' : ''}`}>
                    {/* Large Thumbnail */}
                    <div className="relative aspect-video bg-gradient-to-br from-primary/20 to-primary/5 overflow-hidden">
                      {lesson.thumbnail_url ? <img src={lesson.thumbnail_url} alt={lesson.title} className="w-full h-full object-cover transition-transform group-hover:scale-105" /> : <div className="w-full h-full flex items-center justify-center">
                    ...
                    opacity: ((lesson.banner_config as any)?.overlayOpacity ?? 40) / 100
                          ...
                          {((lesson.banner_config as any)?.title || (lesson.banner_config as any)?.subtitle) && <div className={`absolute inset-0 flex flex-col justify-center px-4 ${(lesson.banner_config as any)?.textAlign === "left" ? "items-start text-left" : (lesson.banner_config as any)?.textAlign === "right" ? "items-end text-right" : "items-center text-center"}`}>
                              {(lesson.banner_config as any)?.title && <h4 className="text-lg sm:text-xl md:text-2xl font-bold leading-tight" style={{
                      color: (lesson.banner_config as any)?.textColor || "#ffffff"
                    }}>
                                  {(lesson.banner_config as any).title}
                                </h4>}
                              {(lesson.banner_config as any)?.subtitle && <p className="mt-1 text-sm sm:text-base opacity-90 line-clamp-2" style={{
                      color: (lesson.banner_config as any)?.textColor || "#ffffff"
                    }}>
                      ...
                      {/* Status overlay */}
                      {(locked || completed) && <div className={`absolute inset-0 flex items-center justify-center ${completed ? lesson.banner_config ? 'bg-green-500/10' : 'bg-green-500/20' : lesson.banner_config ? 'bg-black/20' : 'bg-black/50'}`}>
                          {completed ? <CheckCircle className="h-12 w-12 text-green-500" /> : <Lock className="h-10 w-10 text-white" />}
                        </div>}

                      {/* Play button overlay on hover */}
                      {!locked && !completed && !lesson.banner_config && <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    ...
                    <CardContent className="p-5">
                      <div className="flex items-center gap-2">
                        ...
                        {completed && <Badge className="bg-green-500 gap-1 flex-shrink-0">
                      ...
                      <div className="flex items-center gap-3 mt-3 text-sm text-muted-foreground">
                        {lesson.duration_minutes > 0 && <span className="flex items-center gap-1">
                            <Clock className="h-4 w-4" />
